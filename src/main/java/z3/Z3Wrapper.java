...
public final class Z3Wrapper {
    ...
    private static void loadFromJar() {
        ...
        File libDir  = new File(System.getProperty("java.io.tmpdir") + "/" + path + LIB_BIN);
        ...
        try {
          if (!libDir.isDirectory() || !libDir.canRead()) {
            ...
            extractFromJar(libRealName, libDir);
            extractFromJar(z3LibRealName, libDir);
          }
    ...
    public static void addLibraryPath(String pathToAdd) throws Exception {
        System.setProperty("java.library.path", pathToAdd +":"+System.getProperty("java.library.path"));
    ...
    private static void extractFromJar(String name, File toDir) throws Exception {
        debug("Extracting "+name+" from jar...");

        File to = new File(toDir.getAbsolutePath() + LIB_SEPARATOR + name);

        InputStream in = Z3Wrapper.class.getResourceAsStream(LIB_BIN + name);
        if (in==null)
            throw new java.io.FileNotFoundException("Could not find " + LIB_BIN + name);
